%Ek*wfuna
function re=wfunaK(n,x,y,rx,ry,parameters)
% global d NN hbar;
d=parameters.d;
NN=parameters.NN;
...
for kx=linspace(-xr,xr,NN)
    for ky=linspace(-yc,yc,NN) 
        ...
        ss=ek(n)*exp(-1i*([kx,ky]*([rx,ry])')).*exp(1i*(kx*x+ky*y)).*cor(i).*ua(n,kx,ky,x,y,parameters).*(~((abs(kx)<=xr)&(abs(kx)>=xcr)&(abs(ky)>k*abs(kx)+b)));
            sum= sum+ ss;     
            i=i+1;
    end
end
