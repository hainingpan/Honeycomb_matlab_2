...
% %         end
%         ubra=uaup(Nband,kx,ky,XX,YY,parameters).*((real(uup(Nband,kx,ky,0,0,parameters))<0)*(-1)+(real(uup(Nband,kx,ky,0,0,parameters))>=0));
% %         uket=cell(Nband,1);
% %         for i=1:Nband
% %             uket{i}=um(i,kx+bnx,ky+bny,XX,YY,parameters);
% %         end
%         uket=uaup(Nband,kx+bnx,ky+bny,XX,YY,parameters).*((real(uup(Nband,kx+bnx,ky+bny,0,0,parameters))<0)*(-1)+(real(uup(Nband,kx+bnx,ky+bny,0,0,parameters))>=0));
%         inner_u=zeros(Nband);
%         for i=1:Nband
%             for j=1:Nband
% %                 intu=conj(ubra{i}).*uket{j};
%                 intu=conj(ubra(:,:,i)).*uket(:,:,j);                
%                 self1=conj(ubra(:,:,i)).*ubra(:,:,i);
%                 self2=conj(uket(:,:,j)).*uket(:,:,j);
%                 inner_ubra=trapz(y,trapz(x,self1,2));
%                 inner_uket=trapz(y,trapz(x,self2,2));
%                 inner_u(i,j)=trapz(y,trapz(x,intu,2))/(sqrt(inner_ubra*inner_uket));
%             end
%         end
